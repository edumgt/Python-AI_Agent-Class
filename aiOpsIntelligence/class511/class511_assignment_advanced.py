...
"""
class511 Advanced(심화) 과제 (자기주도 학습용)

[학습 정보]
- 교과목: 프로젝트
- 주제: LLMOps/RAG 서비스 품질관리 · 단계 1/5 입문 이해 [class511]
- 일정: Day 64 / 7교시
- 난이도: 기초응용
...
[실행 방법]
- python aiOpsIntelligence/class511/class511_assignment_advanced.py
...
def advanced_extension():
    ...
    2) 결과를 dict로 정리해 JSON 파일로 저장하세요. (파일명: class511_advanced_result.json)
    ...
    # from pathlib import Path
    # import json
    # Path(__file__).with_name("class511_advanced_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result
