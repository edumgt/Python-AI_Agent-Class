...
차시 목표(요약):
- PromptTemplate 핵심 개념 이해
- PromptTemplate 관련 코드/도구 적용
- PromptTemplate 결과 점검 및 다음 차시 연결
"""
...
def advanced_extension():
    ...
    2) 결과를 dict로 정리해 JSON 파일로 저장하세요. (파일명: class400_advanced_result.json)
    ...
    # from pathlib import Path
    # import json
    # Path(__file__).with_name("class400_advanced_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result
