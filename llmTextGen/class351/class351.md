...
| `시스템` | 명사(주제 핵심 용어) | 시스템 (한자 없음) | (topic-specific) | `시스템`는 `Agent 시스템 통합 구현` 실습에서 생성 결과의 품질·안전성·제어 가능성을 높이는 데 쓰입니다. |
| `통합` | 명사(주제 핵심 용어) | 통합 (한자 없음) | (topic-specific) | 이번 차시 맥락: 생성 기능을 서비스형 Agent로 통합해 라우팅, 검증, 모니터링까지 구현하는 마무리 차시입니다. 이를 기준으로 `통합`를 코드와 결과 해석에 연결합니다. |
| `구현` | 명사 | 구현 (具現) | implementation | 설계를 실제 코드와 시스템 동작으로 구체화하는 과정입니다. |
| `규칙` | 명사(주제 핵심 용어) | 규칙 (한자 없음) | (topic-specific) | 이번 차시 맥락: `규칙 기반 + LLM 기반` 하이브리드 설계는 안정성과 유연성을 동시에 확보합니다. 이를 기준으로 `규칙`를 코드와 결과 해석에 연결합니다. |
...
- 복습 연결: 이전에 배운 **Agent 시스템 통합 구현 · 단계 4/6 응용 확장 [class350]** 를 떠올리며, 오늘 **Agent 시스템 통합 구현 · 단계 5/6 실전 검증 [class351]** 와 어떤 점이 이어지는지 비교해 보세요.
