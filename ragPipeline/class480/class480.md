...
- 문법 포인트: 핵심 개념 명사를 중심으로 한 명사구 구조입니다.
- 기술 포인트: 이번 차시는 `검색 품질 개선` 핵심 개념을 코드 구현, 결과 해석, 점검 기준으로 연결합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
...
| `하이브리드` | 명사(주제 핵심 용어) | 하이브리드 (한자 없음) | (topic-specific) | 이번 차시 맥락: `하이브리드 검색`은 키워드 검색과 벡터 검색을 결합해 재현율/정밀도를 균형 있게 맞춥니다. 이를 기준으로 `하이브리드`를 코드와 결과 해석에 연결합니다. |
| `실패` | 명사(주제 핵심 용어) | 실패 (한자 없음) | (topic-specific) | 이번 차시 맥락: 검색 실패 사례를 분석하고 query rewrite, threshold, hybrid 검색으로 성능을 개선하는 차시입니다. 이를 기준으로 `실패`를 코드와 결과 해석에 연결합니다. |
| `사례` | 명사(주제 핵심 용어) | 사례 (한자 없음) | (topic-specific) | 이번 차시 맥락: 검색 실패 사례를 분석하고 query rewrite, threshold, hybrid 검색으로 성능을 개선하는 차시입니다. 이를 기준으로 `사례`를 코드와 결과 해석에 연결합니다. |
| `분석` | 명사 | 분석 (分析) | analysis | 데이터를 분해해 의미 있는 결론을 도출하는 과정입니다. |

## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class479 / 검색 품질 개선 · 단계 5/6 실전 검증 [class479]** (Day 60 / 7교시)
- 복습 연결: 이전에 배운 **검색 품질 개선 · 단계 5/6 실전 검증 [class479]** 를 떠올리며, 오늘 **검색 품질 개선 · 단계 6/6 운영 최적화 [class480]** 와 어떤 점이 이어지는지 비교해 보세요.
