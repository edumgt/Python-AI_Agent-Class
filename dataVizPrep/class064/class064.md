...
## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class063 / 그룹화와 집계** (Day 08 / 7교시)
...
## 5) 오늘의 예제 코드
- 예제 파일: `class064_example.py`
- 실행 명령:
```bash
python dataVizPrep/class064/class064_example.py
```
...
python3 dataVizPrep/class064/class064_example.py
...
### 기술 스택
- 언어: `Python 3`
- 실행: `CLI` (`python dataVizPrep/class064/class064_example.py`)
...
N7["예제 실행: python dataVizPrep/class064/class064_example.py"]
...
## 10) 학습 후 다음에 배울 내용
- 다음 차시: **class065 / 데이터 병합과 변환** (Day 09 / 1교시)
- 미리보기: 다음 차시 전에 **그룹화와 집계** 핵심 코드 1개를 다시 실행해 두면 데이터 병합과 변환 학습이 더 쉬워집니다.
