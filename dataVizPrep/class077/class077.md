...
- 학습 주제 진행: **Agent 시스템 통합 구현 · 단계 1/4 입문 이해 [class077] (총 4시간 중 1시간차)**
- 세부 시퀀스: **37/40**
- 일정: **Day 10 / 5교시**
...
- 기술 포인트: 이번 차시는 `Agent 시스템 통합 구현 · 단계 1/4 입문 이해 [class077]`를 중심으로 같은 주제 내에서 단계적으로 고도화된 구현을 수행합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
| `전처리` | 명사 | 전처리 (前處理) | preprocessing | 원시 데이터를 모델이 다루기 쉬운 형태로 정리하는 단계입니다. |
| `시각화` | 명사 | 시각화 (視覺化) | visualization | 숫자 데이터를 그래프와 차트로 표현해 패턴을 해석하는 과정입니다. |
...
- 복습 연결: 이전에 배운 **Seaborn/실전 차트 해석 · 단계 4/4 운영 최적화 [class076]** 를 떠올리며, 오늘 **Agent 시스템 통합 구현 · 운영 확장 장애 재현과 복구 (차시 37) [class077]** 와 어떤 점이 이어지는지 비교해 보세요.
...
## 5) 오늘의 예제 코드
- 예제 파일: `class077_example.py`
- 실행 명령:
```bash
python dataVizPrep/class077/class077_example.py
```
...
```powershell
cd C:\DevOps\Python-AI_Agent-Class
python .\dataVizPrep\class077\class077.py
python .\dataVizPrep\class077\class077_example.py
python .\dataVizPrep\class077\class077_assignment.py
start .\dataVizPrep\class077\class077_quiz.html
```

### WSL Ubuntu (bash)
```bash
cd /mnt/c/DevOps/Python-AI_Agent-Class
python3 dataVizPrep/class077/class077.py
python3 dataVizPrep/class077/class077_example.py
python3 dataVizPrep/class077/class077_assignment.py
explorer.exe "$(wslpath -w 'dataVizPrep/class077/class077_quiz.html')"
```

### run_class/run_day 스크립트 연동 (WSL bash)
```bash
./run_class.sh class077
...
### 기술 스택
- 언어: `Python 3`
- 실행: `CLI` (`python dataVizPrep/class077/class077_example.py`)
...
N7["예제 실행: python dataVizPrep/class077/class077_example.py"]
