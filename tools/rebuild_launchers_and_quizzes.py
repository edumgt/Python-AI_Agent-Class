...
PROMPT_ENG_QUIZ_BANK = {
    "프롬프트 엔지니어링 개요": {
        "concept": "프롬프트 품질은 명확성·맥락·제약조건에 따라 크게 달라진다.",
        "action": "동일 질문을 모호한 프롬프트와 명확한 프롬프트로 각각 실행해 비교한다.",
        "pitfall": "목표와 제약을 생략해 재현 불가능한 응답을 만든다.",
        "check": "개선 전후 응답을 형식 일관성·정확성 기준으로 점검한다.",
        "outcome": "프롬프트 설계가 결과 품질에 미치는 영향을 설명할 수 있다.",
    },
    "질문 구조화": {
        "concept": "역할·목표·입력·출력형식·제약조건 5요소가 프롬프트 기본 구조다.",
        "action": "입력 변수와 지시문을 분리한 템플릿으로 프롬프트를 작성한다.",
        "pitfall": "구조 없이 지시를 나열해 출력 누락과 형식 불일치를 만든다.",
        "check": "5요소 포함 여부와 템플릿 재사용성을 검증한다.",
        "outcome": "재사용 가능한 구조화 프롬프트를 설계할 수 있다.",
    },
    "역할/맥락 설정": {
        "concept": "역할 부여와 맥락 제공은 응답 톤, 정확도, 범위를 결정한다.",
        "action": "system 프롬프트와 user 프롬프트를 분리해 실행한다.",
        "pitfall": "역할과 맥락을 섞어 지시 충돌이 발생한다.",
        "check": "역할/맥락 변경 전후 응답 품질을 비교한다.",
        "outcome": "시스템/사용자 프롬프트 분리 설계를 적용할 수 있다.",
    },
    "출력 포맷 제어": {
        "concept": "표/JSON, 길이 제한, 어조/문체 제어, 금지 규칙은 출력 안정성 핵심이다.",
        "action": "동일 입력을 표 출력과 JSON 출력으로 각각 생성해 점검한다.",
        "pitfall": "형식 강제 없이 자연어 출력만 받아 후처리 자동화가 깨진다.",
        "check": "스키마 누락, 길이 초과, 금지 규칙 위반을 자동 검사한다.",
        "outcome": "구조화 출력과 형식 제어를 실무에 적용할 수 있다.",
    },
    "예시 기반 학습": {
        "concept": "Zero-shot/One-shot/Few-shot은 예시 제공량에 따라 성능 특성이 달라진다.",
        "action": "같은 작업을 zero/one/few-shot으로 비교 실험한다.",
        "pitfall": "예시 품질 점검 없이 개수만 늘려 오히려 일반화를 해친다.",
        "check": "예시 추가 전후 오답률과 형식 일관성을 비교한다.",
        "outcome": "예시 기반 프롬프트 전략을 상황에 맞게 선택할 수 있다.",
    },
    "단계적 추론 유도": {
        "concept": "Chain-of-thought 개념과 step-by-step 유도는 복잡한 과업 정확도를 높인다.",
        "action": "단일 지시와 단계적 지시를 비교해 중간 검증을 추가한다.",
        "pitfall": "단계를 명시하지 않아 누락·비약 응답을 그대로 수용한다.",
        "check": "중간 단계 체크포인트와 최종 결과 품질을 함께 점검한다.",
        "outcome": "단계적 추론 유도 프롬프트를 설계하고 검증할 수 있다.",
    },
    "평가와 개선": {
        "concept": "실패 패턴 분석과 반복 개선 루프가 프롬프트 성능 고도화의 핵심이다.",
        "action": "실패 응답을 수집해 유형별 원인과 개선안을 기록한다.",
        "pitfall": "단발성 수정만 수행하고 버전별 비교 기록을 남기지 않는다.",
        "check": "v1/v2/v3 버전별 성능 차이를 동일 테스트로 검증한다.",
        "outcome": "프롬프트 개선 프로세스를 체계적으로 운영할 수 있다.",
    },
    "자동화 프롬프트 패턴": {
        "concept": "템플릿화, 버전 관리, 테스트 자동화가 실무 운영 품질을 만든다.",
        "action": "재사용 가능한 템플릿 함수를 만들고 테스트 케이스를 자동 실행한다.",
        "pitfall": "수동 복붙 방식으로 운영해 변경 추적과 회귀 검증이 불가능해진다.",
        "check": "버전 태깅, 변경 이력, 자동 평가 결과를 함께 점검한다.",
        "outcome": "재사용 가능한 프롬프트 패턴 라이브러리를 구축할 수 있다.",
    },
    "도메인 템플릿 작성": {
        "concept": "고객상담/문서요약/코드생성/보고서/교육 콘텐츠는 도메인별 설계가 필요하다.",
        "action": "도메인별 템플릿과 필수 규칙을 분리해 작성한다.",
        "pitfall": "단일 템플릿을 모든 도메인에 적용해 품질 편차가 커진다.",
        "check": "도메인별 테스트 케이스와 품질 기준으로 비교한다.",
        "outcome": "도메인 요구에 맞는 프롬프트 템플릿을 설계할 수 있다.",
    },
    "실전 프롬프트 튜닝": {
        "concept": "동일 작업 개선 비교, 오답 감소, 구조화 출력, 운영 기준 확정이 마무리 핵심이다.",
        "action": "동일 과업에서 여러 프롬프트 버전을 A/B 비교한다.",
        "pitfall": "실전 적용 전 테스트 케이스 검증 없이 템플릿을 배포한다.",
        "check": "시스템/사용자 분리, JSON 형식, 오류 처리, 테스트 통과 여부를 확인한다.",
        "outcome": "실무에 적용 가능한 프롬프트 튜닝 역량을 확보할 수 있다.",
    },
}
...
LANGCHAIN_QUIZ_BANK = {
    ...
    "실전 체인 애플리케이션": {
        "concept": "실전 통합에서는 LangChain 체인 구성에 LangGraph 상태 흐름 제어와 LangSmith 실행 추적을 함께 적용한다.",
        "action": "요약/질의응답/챗봇/외부연동 시나리오를 통합하고 LangGraph 분기 + LangSmith 로그를 점검한다.",
        "pitfall": "개별 체인만 검증하고 상태 전이/추적 로그 없이 운영에 투입한다.",
        "check": "체인 계약, LangGraph 분기 조건, LangSmith 추적 항목(입력/출력/지연/오류)을 함께 검증한다.",
        "outcome": "운영형 LLM 앱에서 워크플로우 제어와 관측성을 동시에 설계할 수 있다.",
    },
}
