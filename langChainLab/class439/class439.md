...
- 기술 포인트: 이번 차시는 `VectorStore 연동` 핵심 개념을 코드 구현, 결과 해석, 점검 기준으로 연결합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
...
| `Retriever` | 영문 기술명/약어 | Retriever (한자 없음) | Retriever | 이번 차시 맥락: VectorStore 연동으로 Retriever 성능을 높이고 RAG 연계 기반을 확립하는 차시입니다. 이를 기준으로 `Retriever`를 코드와 결과 해석에 연결합니다. |
| `RAG` | 약어명사 | RAG (한자 없음) | Retrieval-Augmented Generation | 검색 결과를 근거로 생성 품질과 신뢰도를 높이는 구조입니다. |
| `연계` | 명사(주제 핵심 용어) | 연계 (한자 없음) | (topic-specific) | 이번 차시 맥락: VectorStore 연동으로 Retriever 성능을 높이고 RAG 연계 기반을 확립하는 차시입니다. 이를 기준으로 `연계`를 코드와 결과 해석에 연결합니다. |

## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class438 / VectorStore 연동 · 단계 1/6 입문 이해 [class438]** (Day 55 / 6교시)
- 복습 연결: 이전에 배운 **VectorStore 연동 · 단계 1/6 입문 이해 [class438]** 를 떠올리며, 오늘 **VectorStore 연동 · 단계 2/6 기초 구현 [class439]** 와 어떤 점이 이어지는지 비교해 보세요.
...
N8["다음 준비: VectorStore 연동 · 단계 3/6 응용 확장 [class440] 연결 포인트 정리"]
N1 --> N2
N2 --> N3
N3 --> N4
N4 --> N5
