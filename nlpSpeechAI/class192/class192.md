...
## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class191 / 특징 추출(MFCC 등)** (Day 24 / 7교시)
...
## 5) 오늘의 예제 코드
- 예제 파일: `class192_example.py`
- 실행 명령:
```bash
python nlpSpeechAI/class192/class192_example.py
```
...
python .\nlpSpeechAI\class192\class192_example.py
...
python3 nlpSpeechAI/class192/class192_example.py
...
### 기술 스택
- 언어: `Python 3`
- 실행: `CLI` (`python nlpSpeechAI/class192/class192_example.py`)
...
N7["예제 실행: python nlpSpeechAI/class192/class192_example.py"]
...
## 10) 학습 후 다음에 배울 내용
- 다음 차시: **class193 / 텍스트 분류 기초** (Day 25 / 1교시)
- 미리보기: 다음 차시 전에 **특징 추출(MFCC 등)** 핵심 코드 1개를 다시 실행해 두면 텍스트 분류 기초 학습이 더 쉬워집니다.
