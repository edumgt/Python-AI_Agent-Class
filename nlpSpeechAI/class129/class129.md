...
- 기술 포인트: 이번 차시는 `NLP/STT/TTS 개요` 핵심 개념을 코드 구현, 결과 해석, 점검 기준으로 연결합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
...
| `데이터-전처리-특징추출-모델학습-평가-배포` | 명사(주제 핵심 용어) | 데이터-전처리-특징추출-모델학습-평가-배포 (한자 없음) | (topic-specific) | 이번 차시 맥락: `학습 데이터-전처리-특징추출-모델학습-평가-배포` 흐름이 NLP/Speech 공통 개발 사이클입니다. 이를 기준으로 `데이터-전처리-특징추출-모델학습-평가-배포`를 코드와 결과 해석에 연결합니다. |
| `데이터` | 명사(외래어) | 데이터 (한자 없음) | data | 분석, 학습, 추론의 입력이 되는 관측값 집합입니다. |

## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class128 / Agent 시스템 통합 구현 · 단계 4/4 운영 최적화 [class128]** (Day 16 / 8교시)
- 복습 연결: 이전에 배운 **Agent 시스템 통합 구현 · 단계 4/4 운영 최적화 [class128]** 를 떠올리며, 오늘 **NLP/STT/TTS 개요 · 단계 1/8 입문 이해 [class129]** 와 어떤 점이 이어지는지 비교해 보세요.
