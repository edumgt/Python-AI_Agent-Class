...
"""
project004 Advanced(심화) 과제 (자기주도 학습용)

[학습 정보]
- 교과목: 프로젝트
- 주제: DevOps 프로젝트 착수와 요구사항 정의 · 단계 4/5 실전 검증 [project004]
- 일정: Day 63 / 8교시
- 난이도: 실전심화
...
[실행 방법]
- python project/Prj_DevOpsKickoff/project004/project004_assignment_advanced.py
...
def advanced_extension():
    ...
    2) 결과를 dict로 정리해 JSON 파일로 저장하세요. (파일명: project004_advanced_result.json)
    ...
    # from pathlib import Path
    # import json
    # Path(__file__).with_name("project004_advanced_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result
