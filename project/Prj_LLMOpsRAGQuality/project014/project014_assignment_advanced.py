...
"""
project014 Advanced(심화) 과제 (자기주도 학습용)

[학습 정보]
- 교과목: 프로젝트
- 주제: LLMOps/RAG 서비스 품질관리 · 단계 4/5 실전 검증 [project014]
- 일정: Day 65 / 2교시
- 난이도: 실전심화
...
[실행 방법]
- python project/Prj_LLMOpsRAGQuality/project014/project014_assignment_advanced.py
...
def advanced_extension():
    ...
    2) 결과를 dict로 정리해 JSON 파일로 저장하세요. (파일명: project014_advanced_result.json)
    ...
    # from pathlib import Path
    # import json
    # Path(__file__).with_name("project014_advanced_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result
