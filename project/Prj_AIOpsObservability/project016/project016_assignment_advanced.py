...
"""
project016 Advanced(심화) 과제 (자기주도 학습용)

[학습 정보]
- 교과목: 프로젝트
- 주제: AIOps 관측성·이상탐지·자동복구 · 단계 1/5 입문 이해 [project016]
- 일정: Day 65 / 4교시
- 난이도: 기초응용
...
[실행 방법]
- python project/Prj_AIOpsObservability/project016/project016_assignment_advanced.py
...
def advanced_extension():
    ...
    2) 결과를 dict로 정리해 JSON 파일로 저장하세요. (파일명: project016_advanced_result.json)
    ...
    # from pathlib import Path
    # import json
    # Path(__file__).with_name("project016_advanced_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result
