...
"""
project010 Advanced(심화) 과제 (자기주도 학습용)

[학습 정보]
- 교과목: 프로젝트
- 주제: MLOps 파이프라인과 모델 레지스트리 · 단계 5/5 운영 최적화 [project010]
- 일정: Day 64 / 6교시
- 난이도: 실전심화
...
[실행 방법]
- python project/Prj_MLOpsPipeline/project010/project010_assignment_advanced.py
...
def advanced_extension():
    ...
    2) 결과를 dict로 정리해 JSON 파일로 저장하세요. (파일명: project010_advanced_result.json)
    ...
    # from pathlib import Path
    # import json
    # Path(__file__).with_name("project010_advanced_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result
