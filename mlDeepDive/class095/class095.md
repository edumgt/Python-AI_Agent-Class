...
## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class094 / 회귀 모델** (Day 12 / 6교시)
- 복습 연결: 이전에 배운 **회귀 모델** 를 떠올리며, 오늘 **회귀 모델** 와 어떤 점이 이어지는지 비교해 보세요.
...
## 10) 학습 후 다음에 배울 내용
- 다음 차시: **class096 / 분류 모델** (Day 12 / 8교시)
- 미리보기: 다음 차시 전에 **회귀 모델** 핵심 코드 1개를 다시 실행해 두면 분류 모델 학습이 더 쉬워집니다.
