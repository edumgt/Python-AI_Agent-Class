...
- 기술 포인트: 이번 차시는 `지도학습 기초` 핵심 개념을 코드 구현, 결과 해석, 점검 기준으로 연결합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
| `지도학습` | 명사 | 지도학습 (指導學習) | supervised learning | 라벨이 있는 데이터를 사용해 입력-정답 관계를 학습하는 머신러닝 방식입니다. |
| `비지도학습` | 명사 | 비지도학습 (非指導學習) | unsupervised learning | 정답 라벨 없이 데이터 구조와 군집 패턴을 스스로 찾는 학습 방식입니다. |
...
- 복습 연결: 이전에 배운 **ML/DL 개요와 문제정의 · 단계 5/5 운영 최적화 [class085]** 를 떠올리며, 오늘 **지도학습 기초 · 단계 1/5 입문 이해 [class086]** 와 어떤 점이 이어지는지 비교해 보세요.
