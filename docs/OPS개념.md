...
|---|---|---|---|
| VoiceModelBuilder (`project001`) | 나만의 음성 모델 만들기 | DevOps + MLOps | 음성 프로필/학습 파이프라인 구성, API 서빙 및 실행 자동화 |
| PersonaLLMResponder (`project002`) | 거대 언어 모델 기반 PERSONA AI 답변 구현 | LLMOps / AI Engineering | 페르소나 규칙 설계, 프롬프트 기반 응답, LLM/로컬 fallback 운영 |
| PersonaKnowledgeCustomizer (`project003`) | 사전 데이터 기반 PERSO 답변 커스텀 | LLMOps + RAG + AIOps | 지식 업서트/검색/근거 응답, 로그 기반 품질 점검 및 개선 루프 |
