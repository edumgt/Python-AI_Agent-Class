...
## 8. 커리큘럼 접목 점검 및 반영 (project001~project020)

현재 저장소 커리큘럼은 `project001`부터 `project020`까지를 **프로젝트 과목**으로 분류하고, 본 보고서의 핵심 축(DevOps/MLOps/AIOps/LLMOps)에 맞춰 다음과 같이 접목했다.

| 클래스 범위 | 프로젝트 주제 | 보고서 접목 축 | 핵심 반영 포인트 |
|---|---|---|---|
| project001~project005 | DevOps 프로젝트 착수와 요구사항 정의 | DevOps | 서비스 범위 정의, 배포 대상/운영 기준 정렬 |
| project006~project010 | MLOps 파이프라인과 모델 레지스트리 | MLOps + DevOps | 모델 버전/레지스트리, 실험-배포 자동화 |
| project011~project015 | LLMOps/RAG 서비스 품질관리 | LLMOps / AI Engineering | 프롬프트/RAG 품질 평가, 가드레일, 운영 품질관리 |
| project016~project020 | AIOps 관측성·이상탐지·자동복구 | AIOps + Observability | 로그·메트릭·트레이스 기반 탐지/분석/복구 자동화 |

정리하면, project001~project020 프로젝트 과목은 본 보고서 2장(개념 비교), 4장(영역 포함), 5장(실무 요약), 7장(핵심 요약)을 실제 학습 시퀀스로 연결한 운영형 프로젝트 트랙이다.
