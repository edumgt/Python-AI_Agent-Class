...
- 문법 포인트: 핵심 개념 명사를 중심으로 한 명사구 구조입니다.
- 기술 포인트: 이번 차시는 `수업 준비 3: 개발환경 검증/실행 확인 (class003)` 핵심 개념을 코드 구현, 결과 해석, 점검 기준으로 연결합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
| `개발환경` | 명사(주제 핵심 용어) | 개발환경 (한자 없음) | (topic-specific) | 이번 차시 맥락: `VS Code + 가상환경(venv) + pip` 조합은 실습 재현성과 패키지 충돌 방지의 기본 개발환경입니다. 이를 기준으로 `개발환경`를 코드와 결과 해석에 연결합니다. |
| `검증` | 명사 | 검증 (檢證) | validation | 결과가 요구사항과 기준을 만족하는지 확인하는 절차입니다. |
| `실행` | 명사(주제 핵심 용어) | 실행 (한자 없음) | (topic-specific) | 이번 차시 맥락: Python 코드가 어떤 실행환경에서 동작하는지 먼저 맞추는 차시입니다. 이를 기준으로 `실행`를 코드와 결과 해석에 연결합니다. |
| `확인` | 명사(주제 핵심 용어) | 확인 (한자 없음) | (topic-specific) | 이번 차시 맥락: 코드가 `__name__ == "__main__"` 블록에서 시작되는지 확인하기. 이를 기준으로 `확인`를 코드와 결과 해석에 연결합니다. |
| `Python` | 고유명사(언어명) | Python (한자 없음) | Python | 데이터 처리와 AI 실습에 널리 쓰이는 범용 프로그래밍 언어입니다. |
...
- 복습 연결: 이전에 배운 **수업 준비 2: 필수 소프트웨어 설치 (class002) · 단계 1/1 입문 이해 [class002]** 를 떠올리며, 오늘 **수업 준비 3: 개발환경 검증/실행 확인 (class003) · 단계 1/1 입문 이해 [class003]** 와 어떤 점이 이어지는지 비교해 보세요.
