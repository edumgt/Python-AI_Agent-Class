...
## 1) 오늘의 학습 정보
- 교과목: **Python 프로그래밍**
- 학습 주제: **연산자와 조건문 · 단계 3/4 실전 검증 [class011]**
- 학습 주제 진행: **연산자와 조건문 · 단계 3/4 실전 검증 [class011] (총 4시간 중 3시간차)**
- 세부 시퀀스: **11/40**
- 일정: **Day 02 / 3교시**
...
#### 학습주제 표현 분석: `연산자와 조건문 · 단계 3/4 실전 검증 [class011]`
- 문법 포인트: 명사와 명사를 대등하게 묶는 병렬 명사구 구조입니다.
- 기술 포인트: 이번 차시는 `연산자와 조건문 · 단계 3/4 실전 검증 [class011]`를 중심으로 같은 주제 내에서 단계적으로 고도화된 구현을 수행합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
...
## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class010 / 연산자와 조건문 · 단계 2/4 기초 구현 [class010]** (Day 02 / 2교시)
- 복습 연결: 이전에 배운 **연산자와 조건문 · 단계 2/4 기초 구현 [class010]** 를 떠올리며, 오늘 **연산자와 조건문 · 핵심 구현 성능 병목 측정 (차시 11) [class011]** 와 어떤 점이 이어지는지 비교해 보세요.

## 3) 주제를 아주 쉽게 이해하기
- 한 줄 설명: 연산자와 조건문를 단계 3/4(실전 검증) 수준으로 고도화해 구현하는 차시입니다.
- 왜 배우나요?: 동일 주제를 반복하더라도 단계별 난이도를 높여 실무 수준의 문제 해결력을 만들기 위해서입니다.

### 핵심 개념 3가지
1. `연산자와 조건문`의 핵심 입력/출력 구조를 단계 3/4 기준으로 명확히 정의합니다.
...
## 5) 오늘의 예제 코드
- 예제 파일: `class011_example1.py`
- 실행 명령:
```bash
python pyBasics/class011/class011_example1.py
```
...
python3 pyBasics/class011/class011_example1.py
...
### 기술 스택
- 언어: `Python 3`
- 실행: `CLI` (`python pyBasics/class011/class011_example1.py`)
- 주요 문법: `산술/비교/논리 연산자`, `if / elif / else`, `단락 평가(and, or)`, `멤버십(in, not in)`
- 학습 포커스: `연산자와 조건문 · 단계 3/4 실전 검증 [class011]`

### 실습 example1.py 동작 원리 (Mermaid Flowchart)
```mermaid
flowchart TD
N1["시작: class011 (11/40, 기초응용)"]
N2["학습 주제 파악: 연산자와 조건문 · 단계 3/4 실전 검증 [class011]"]
N3["1단계: 요구사항을 불리언 조건식으로 변환한다"]
N4["2단계: 연산자 우선순위를 정리해 조건식을 확정한다"]
N5["3단계: if/elif/else로 분기 경로를 구현한다"]
N6["4단계: 경계값 테스트로 분기 누락 여부를 검증한다"]
N7["예제 실행: python pyBasics/class011/class011_example1.py"]
N8["다음 준비: 연산자와 조건문 · 단계 3/4 실전 검증 [class011] 연결 포인트 정리"]
N1 --> N2
N2 --> N3
N3 --> N4
N4 --> N5
...
### 실습 미션
1. `연산자와 조건문` 단계 3/4 목표 기능을 코드로 구현하고 실행 로그를 남기세요.
...
## 10) 학습 후 다음에 배울 내용
- 다음 차시: **class012 / 연산자와 조건문 · 단계 4/4 운영 최적화 [class012]** (Day 02 / 4교시)
- 미리보기: 다음 차시 전에 **연산자와 조건문 · 단계 3/4 실전 검증 [class011]** 핵심 코드 1개를 다시 실행해 두면 연산자와 조건문 · 단계 4/4 운영 최적화 [class012] 학습이 더 쉬워집니다.
