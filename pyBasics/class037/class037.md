...
## 1) 오늘의 학습 정보
- 교과목: **Python 프로그래밍**
- 학습 주제: **FastAPI와 Uvicorn · 단계 1/2 입문 이해 [class037]**
- 학습 주제 진행: **FastAPI와 Uvicorn · 단계 1/2 입문 이해 [class037] (총 2시간 중 1시간차)**
- 세부 시퀀스: **37/40**
- 일정: **Day 05 / 5교시**
...
#### 학습주제 표현 분석: `FastAPI와 Uvicorn · 단계 1/2 입문 이해 [class037]`
- 문법 포인트: 핵심 개념 명사를 조합한 실무형 기술 주제 표현입니다.
- 기술 포인트: 이번 차시는 `FastAPI와 Uvicorn · 단계 1/2 입문 이해 [class037]`를 중심으로 같은 주제 내에서 단계적으로 고도화된 구현을 수행합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
| `FastAPI와 Uvicorn` | 명사구(복합 기술어) | 혼합 표기 | practical topic | 실무 API 개발에서 즉시 적용 가능한 핵심 범위입니다. |

## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class036 / API 기본 개념과 HTTP · 단계 2/2 운영 최적화 [class036]** (Day 05 / 4교시)
- 복습 연결: 이전에 배운 **API 기본 개념과 HTTP · 단계 2/2 운영 최적화 [class036]** 를 떠올리며, 오늘 **FastAPI와 Uvicorn · 운영 확장 장애 재현과 복구 (차시 37) [class037]** 와 어떤 점이 이어지는지 비교해 보세요.

## 3) 주제를 아주 쉽게 이해하기
- 한 줄 설명: FastAPI와 Uvicorn를 단계 1/2(입문 이해) 수준으로 고도화해 구현하는 차시입니다.
- 왜 배우나요?: 동일 주제를 반복하더라도 단계별 난이도를 높여 실무 수준의 문제 해결력을 만들기 위해서입니다.

### 핵심 개념 3가지
1. `FastAPI와 Uvicorn`의 핵심 입력/출력 구조를 단계 1/2 기준으로 명확히 정의합니다.
...
### 기술 스택
- 언어: `Python 3`
- 실행: `Uvicorn` (`uvicorn pyBasics.class037.class037:app --reload`)
...
N2["학습 주제 파악: FastAPI와 Uvicorn · 단계 1/2 입문 이해 [class037]"]
...
### 실습 미션
1. `FastAPI와 Uvicorn` 단계 1/2 목표 기능을 코드로 구현하고 실행 로그를 남기세요.
...
## 10) 학습 후 다음에 배울 내용
- 다음 차시: **class038 / FastAPI와 Uvicorn · 단계 2/2 운영 최적화 [class038]** (Day 05 / 6교시)
- 미리보기: 다음 차시 전에 **FastAPI와 Uvicorn · 단계 1/2 입문 이해 [class037]** 핵심 코드 1개를 다시 실행해 두면 FastAPI와 Uvicorn · 단계 2/2 운영 최적화 [class038] 학습이 더 쉬워집니다.
