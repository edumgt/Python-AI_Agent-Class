...
### `venv`는 어떤 폴더이고 왜 필요할까요?
- `.venv/`는 이 프로젝트 전용 Python 환경 폴더입니다.
- 이 폴더 안에는 전용 Python 실행 파일과 전용 라이브러리가 들어 있습니다.
- 즉, 내 PC의 전체 Python(전역 환경)과 분리된 "수업 전용 작업공간"입니다.

필요한 이유:
1. 프로젝트마다 다른 패키지 버전 충돌을 막을 수 있습니다.
2. `requirements.txt` 기준으로 같은 실습 환경을 다시 만들 수 있습니다.
3. 문제가 생겨도 `.venv`를 지우고 다시 만들면 복구가 쉬워집니다.

### Python 실행 후 생기는 폴더가 궁금해요
- `__pycache__/`: Python이 실행 속도를 높이기 위해 만드는 캐시 폴더입니다.
- `*.pyc`: `__pycache__/` 안에 저장되는 바이트코드 파일입니다.
- `.venv/`: 우리가 만든 프로젝트 전용 Python 실행 환경입니다.

알아두기:
1. `__pycache__`는 정상입니다(오타로 `__pychach`라고 부르는 경우가 있음).
2. 캐시 폴더를 삭제해도 Python을 다시 실행하면 자동으로 다시 생성됩니다.
