# 라인 1: 기존 설명 주석으로, 아래 코드의 의도나 맥락을 안내합니다.
# 이 파일은 www.edumgt.co.kr 의 에듀엠지티에 저작권이 있습니다

# 라인 2: 가독성을 위해 구분한 빈 줄입니다.

# 라인 3: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
"""class001 example3: 오리엔테이션 및 개발환경 준비"""

# 라인 4: 가독성을 위해 구분한 빈 줄입니다.

# 라인 5: 값을 변수에 저장하거나 갱신해 이후 로직에서 재사용합니다.
TOPIC = "오리엔테이션 및 개발환경 준비"
# 라인 6: 값을 변수에 저장하거나 갱신해 이후 로직에서 재사용합니다.
EXAMPLE_TEMPLATE = "dev_setup"

# 라인 7: 가독성을 위해 구분한 빈 줄입니다.

# 라인 8: 재사용 가능한 함수 동작을 정의하는 함수 선언입니다.
def checklist():
    # 라인 9: 함수 실행 결과를 호출한 위치로 반환하고 함수 실행을 종료합니다.
    return [
        # 라인 10: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
        "python -m venv .venv",
        # 라인 11: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
        "가상환경 활성화",
        # 라인 12: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
        "pip install -r requirements.txt",
    # 라인 13: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
    ]

# 라인 14: 가독성을 위해 구분한 빈 줄입니다.

# 라인 15: 재사용 가능한 함수 동작을 정의하는 함수 선언입니다.
def main():
    # 라인 16: 계산 결과나 안내 메시지를 콘솔에 출력합니다.
    print("오늘 주제:", TOPIC)
    # 라인 17: 반복 가능한 데이터를 순회하며 같은 작업을 반복합니다.
    for idx, step in enumerate(checklist(), start=1):
        # 라인 18: 계산 결과나 안내 메시지를 콘솔에 출력합니다.
        print(f"{idx}. {step}")

# 라인 19: 가독성을 위해 구분한 빈 줄입니다.


# 라인 20: 가독성을 위해 구분한 빈 줄입니다.

# 라인 21: 재사용 가능한 함수 동작을 정의하는 함수 선언입니다.
def self_check():
    # 라인 22: 함수 실행 결과를 호출한 위치로 반환하고 함수 실행을 종료합니다.
    return [
        # 라인 23: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
        "입력/출력 형식을 다시 설명할 수 있는가?",
        # 라인 24: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
        "오류 상황 1가지를 직접 만들어 테스트했는가?",
        # 라인 25: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
        "핵심 로직을 함수 단위로 분리했는가?",
    # 라인 26: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
    ]

# 라인 27: 가독성을 위해 구분한 빈 줄입니다.

# 라인 28: 재사용 가능한 함수 동작을 정의하는 함수 선언입니다.
def challenge_case():
    # 라인 29: 함수 실행 결과를 호출한 위치로 반환하고 함수 실행을 종료합니다.
    return {
        # 라인 30: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
        "task": "같은 로직을 새로운 입력 데이터로 재실행",
        # 라인 31: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
        "goal": "결과 차이를 한 문장으로 요약",
    # 라인 32: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
    }

# 라인 33: 가독성을 위해 구분한 빈 줄입니다.

# 라인 34: 파일을 직접 실행했을 때만 아래 실행 블록이 동작하도록 분기합니다.
if __name__ == "__main__":
    # 라인 35: 아래 코드 한 줄을 단계적으로 실행해 현재 학습 개념을 보여줍니다.
    main()
    # 라인 36: 계산 결과나 안내 메시지를 콘솔에 출력합니다.
    print("자가 점검:", self_check())
    # 라인 37: 계산 결과나 안내 메시지를 콘솔에 출력합니다.
    print("챌린지:", challenge_case())
