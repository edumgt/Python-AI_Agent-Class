...
"""class027 example5: 파일 입출력 · 단계 3/4 실전 검증 [class027]"""

TOPIC = "파일 입출력 · 단계 3/4 실전 검증 [class027]"
...
def run_automation(base_dir):
    text_path = base_dir / "class027_summary.txt"
    csv_path = base_dir / "class027_scores.csv"
    rows = build_rows()
...
def ops_readiness_check():
    return {
        "risk": "파일 손상/권한 오류 시 복구 정책을 정의하세요.",
