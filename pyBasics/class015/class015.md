...
#### 학습주제 표현 분석: `반복문과 흐름제어`
- 문법 포인트: 명사와 명사를 대등하게 묶는 병렬 명사구 구조입니다.
- 기술 포인트: 이번 차시는 `반복문과 흐름제어` 용어를 중심으로 문제 정의, 코드 구현, 결과 검증까지 연결합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
...
## 2) 이전에 배운 내용 (복습)
- 이전 차시: **class014 / 반복문과 흐름제어** (Day 02 / 6교시)
- 복습 연결: 이전에 배운 **반복문과 흐름제어** 를 떠올리며, 오늘 **반복문과 흐름제어** 와 어떤 점이 이어지는지 비교해 보세요.
...
3. `누적 변수(accumulator)` 패턴은 합계/카운트/최댓값 같은 집계 로직의 기본입니다.
...
3. 합계·평균·개수 집계를 누적 변수로 작성하고 결과를 검증하세요.
...
## 10) 학습 후 다음에 배울 내용
- 다음 차시: **class016 / 반복문과 흐름제어** (Day 02 / 8교시)
- 미리보기: 다음 차시 전에 **반복문과 흐름제어** 핵심 코드 1개를 다시 실행해 두면 반복문과 흐름제어 학습이 더 쉬워집니다.
