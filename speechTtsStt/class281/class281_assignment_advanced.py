...
"""
class281 Advanced(심화) 과제 (자기주도 학습용)
...
[실행 방법]
- python speechTtsStt/class281/class281_assignment_advanced.py
...
def advanced_extension():
    ...
    2) 결과를 dict로 정리해 JSON 파일로 저장하세요. (파일명: class281_advanced_result.json)
    ...
    # from pathlib import Path
    # import json
    # Path(__file__).with_name("class281_advanced_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    return result
