...
- 학습 주제 진행: **질문 구조화 · 단계 1/4 입문 이해 [class357] (총 4시간 중 1시간차)**
- 세부 시퀀스: **5/40**
- 일정: **Day 45 / 5교시**
...
- 문법 포인트: 핵심 개념 명사를 중심으로 한 명사구 구조입니다.
- 기술 포인트: 이번 차시는 `질문 구조화 · 단계 1/4 입문 이해 [class357]`를 중심으로 같은 주제 내에서 단계적으로 고도화된 구현을 수행합니다.
| 용어 | 문법/품사 | 한글·한자 | 영어 | 기술 설명 |
| --- | --- | --- | --- | --- |
...
- 복습 연결: 이전에 배운 **프롬프트 엔지니어링 개요 · 단계 4/4 운영 최적화 [class356]** 를 떠올리며, 오늘 **질문 구조화 · 기초 정리 모듈 경계 설정 (차시 05) [class357]** 와 어떤 점이 이어지는지 비교해 보세요.

## 3) 주제를 아주 쉽게 이해하기
- 한 줄 설명: 질문 구조화를 단계 1/4(입문 이해) 수준으로 고도화해 구현하는 차시입니다.
- 왜 배우나요?: 동일 주제를 반복하더라도 단계별 난이도를 높여 실무 수준의 문제 해결력을 만들기 위해서입니다.

### 핵심 개념 3가지
1. `질문 구조화`의 핵심 입력/출력 구조를 단계 1/4 기준으로 명확히 정의합니다.
...
## 5) 오늘의 예제 코드
- 예제 파일: `class357_example.py`
- 실행 명령:
```bash
python promptEng/class357/class357_example.py
```
...
```powershell
cd C:\DevOps\Python-AI_Agent-Class
python .\promptEng\class357\class357.py
python .\promptEng\class357\class357_example.py
python .\promptEng\class357\class357_assignment.py
start .\promptEng\class357\class357_quiz.html
```

### WSL Ubuntu (bash)
```bash
cd /mnt/c/DevOps/Python-AI_Agent-Class
python3 promptEng/class357/class357.py
python3 promptEng/class357/class357_example.py
python3 promptEng/class357/class357_assignment.py
explorer.exe "$(wslpath -w 'promptEng/class357/class357_quiz.html')"
```

### run_class/run_day 스크립트 연동 (WSL bash)
```bash
./run_class.sh class357
...
### 기술 스택
- 언어: `Python 3`
- 실행: `CLI` (`python promptEng/class357/class357_example.py`)
...
N7["예제 실행: python promptEng/class357/class357_example.py"]
...
### 실습 미션
1. `질문 구조화` 단계 1/4 목표 기능을 코드로 구현하고 실행 로그를 남기세요.
